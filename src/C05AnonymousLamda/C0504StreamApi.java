...
////      findFirst : 첫번째요소 반환
//        String myString2 = Arrays.stream(stArr).filter(a -> a.length()>=5).findFirst().get();
//        System.out.println("findFirst : " + myString2);
...
//
////        2) 가장 나이 어린사람 찾기(정렬)
//        Student st = students.stream().sorted((o1, o2) -> o1.getAge()-o2.getAge()).findFirst().get();
//
...
//        Optional<String> opt2_1 = Optional.ofNullable("hello");   // 값이있는 optional 객체 생성
//        Optional<String> opt3 = Optional.of("hello");   // 값이 있는 경우에만 optional.of
//
////      **중요** optional 객체 처리 방버 4가지
...
//            System.out.println("값이 없습니다.");
//        }
////       방법2. orElse() : 값이 있으면 있는값 return, 없으면 저장값 return
...
////        개발에서 사용자에게 적절한 메세지 전달 목적으로 의도된 코드중단을 목표로 강제로 예외를 발생시키는 경우가 많음.
...
//        student가 있으면 student 출력, 없으면 "해당학생은 없습니다." 라고 에러메시지와 함게 에러 강제 발생
